...
All **Technology Adapter Interface** implementations are expected to throw a [`std::runtime_error`](https://en.cppreference.com/w/cpp/error/runtime_error) or any of it's children in case requested operation could not be carried out or to completely remove the `Device` instance by deregistering it. 
