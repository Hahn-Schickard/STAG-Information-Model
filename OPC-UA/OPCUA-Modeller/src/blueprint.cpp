...
void BluePrint::AddDeviceGroup(string name, string desc) {
	
	auto groupRefId = generateRefId(this->device->getReferenceId());
	DeviceElementGroup * group = new DeviceElementGroup(groupRefId, name, desc);
	this->device->addDeviceElementGroup(group);
}
void BluePrint::AddElement(string name, string desc, ElementType elementType) {
