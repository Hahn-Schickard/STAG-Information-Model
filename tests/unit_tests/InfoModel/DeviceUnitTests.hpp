#ifndef DEVICE_UNIT_TESTS_HPP
#define DEVICE_UNIT_TESTS_HPP
...
public:
  Device *device;
  DeviceElementGroup *deviceElementGroup =
      new DeviceElementGroup("123", "", "");

  DeviceTest() {}

  virtual void SetUp() {
    device = new Device("1234", "Temperatur", "Sensor vom Typ Temperature");
    deviceElementGroup = new DeviceElementGroup(
        device->getReferenceId() + ".1", device->getElementName() + " group",
        "Group Element of " + device->getElementName());
  }

  virtual void TearDown() { delete device; }
};
...
TEST(
    Device,
    CreatingDevice_InvalidDeviceParameter_ThrowsInvalidDeviceParameterException) {
  EXPECT_THROW(
      {
        Device *device =
            new Device("1234a", "Temperatur", "Sensor vom Typ Temperatur");
      },
...
TEST_F(DeviceTest,
       AddingSubElementToGroup_ValidParameters_DeviceMetricIsAddedToGroup) {

  // Arrange
  DeviceMetric *temperature =
      new DeviceMetric(device->getReferenceId() + ".1.1", "Temperature",
                       "Temperature Metric Element with ValueType as Float",
  ...
  ASSERT_EQ(1, g1->getSubElements().size());

  DeviceElement expectedDeviceElement = g1->getSubElements()[0];
  EXPECT_EQ("1234.1.1", expectedDeviceElement.getReferenceId());
  EXPECT_EQ("Temperature", expectedDeviceElement.getElementName());
  EXPECT_EQ("Temperature Metric Element with ValueType as Float",
            expectedDeviceElement.getElementDescription());
}

TEST_F(
    DeviceTest,
    AddingSubElementObservable_ValidParameters_ObservableMetricIsAddedToGroup) {

  // Act
  ObservableMetric *temperature =
      new ObservableMetric(device->getReferenceId() + ".1.1", "Temperature",
                           "Temperature Metric Element with ValueType as Float",
                           ValueType::ValueDataType::Float);
  deviceElementGroup->addElement(*temperature);
  device->addDeviceElementGroup(deviceElementGroup);
  ...
  DeviceElement actualDeviceElement = g1->getSubElements()[0];
  EXPECT_EQ("1234.1.1", actualDeviceElement.getReferenceId());
  EXPECT_EQ("Temperature", actualDeviceElement.getElementName());
  EXPECT_EQ("Temperature Metric Element with ValueType as Float",
            actualDeviceElement.getElementDescription());
}

TEST_F(DeviceTest,
       AddingSubElementReadonly_ValidParameters_WriteableMetricIsAddedToGroup) {

  // Act
  DeviceMetric temperature =
      DeviceMetric(device->getReferenceId() + ".1.1", "Temperature",
                   "Temperature Metric Element with ValueType as Float",
                   ValueType::ValueDataType::Float);
  deviceElementGroup->addElement(temperature);
  device->addDeviceElementGroup(deviceElementGroup);
...
TEST_F(
    DeviceTest,
    AddingSubElementWriteable_ValidParameters_WriteableMetricIsAddedToGroup) {
  // Arrange

  WriteableMetric *temperature =
      new WriteableMetric(device->getReferenceId() + ".1.1", "Temperature",
                          "Temperature Metric Element with ValueType as Float",
                          ValueType::ValueDataType::Float);
  deviceElementGroup->addElement(*temperature);
...
TEST_F(DeviceTest, AddingFunctionToGroup_ValidParameters_DeviceMetricAdded) {
  ...
  DeviceFunction *deviceFunction =
      new DeviceFunction(device->getReferenceId() + ".1.3", "GetTemperature",
  ...
  EXPECT_EQ("Returns the Temperature Value",
            actualDeviceElement.getElementDescription());
}

#endif // DEVICE_UNIT_TESTS_HPP